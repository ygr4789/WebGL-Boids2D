...
function Boid() {
  ...
  this.ax = 0;
  this.ay = 0;
  this.color = [0.3, Math.random(), Math.random(), 1];
...
function zeor() {
  ...
  gl.uniform4fv(colorUniformLocation, [0.3, Math.random(), Math.random(), 1]);
...
function render() {
  ...
  for (let boid of boids) {
    ...
    let cf1 = (boid.x - margin) / (Xmax - 2 * margin);
    let cf2 = (boid.y - margin) / (Ymax - 2 * margin);
    let s1 = (Math.sin(cf1) + 1) / 2;
    let s2 = (Math.sin(cf2) + 1) / 2;
    let s = (s1 + s2) / 2;
    gl.uniform4fv(colorUniformLocation, [s1, 0, 1, 1]);
...
function animate(now) {
  ...
  then = now;
  // not used due to timestep jumps when tab changes

  updateBoidsVelocity(timeStep);
  render();
